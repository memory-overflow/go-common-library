...
type queueContainer struct {
	...
	stopedTaskMap    sync.Map // 暂停的任务 map， taskId -> framework.Task
	runningTaskMap   sync.Map // 运行中的任务的 map， taskId -> framework.Task
	runningTaskCount int32    // 运行中的任务总数

	waitingTasks chan framework.Task
...
func MakeQueueContainer(size uint32, timeout time.Duration) *queueContainer {
	return &queueContainer{
		waitingTasks: make(chan framework.Task, size),
		timeout:      timeout,
	}
}

// AddTask 添加任务
func (q *queueContainer) AddTask(ctx context.Context, task framework.Task) (err error) {
	// 如果是之前已经暂停，那么直接删除暂停状态
	if _, ok := q.stopedTaskMap.LoadAndDelete(task.TaskId); ok {
		return
	}
	task.TaskStatus = framework.TASK_STATUS_WAITING
	select {
...
func (q *queueContainer) AddRunningTask(ctx context.Context, task framework.Task) (err error) {
	// 如果是之前已经暂停，那么直接删除暂停状态
...
func (q *queueContainer) GetRunningTask(ctx context.Context) (tasks []framework.Task, err error) {
	q.runningTaskMap.Range(
		func(key, value interface{}) bool {
			task := value.(framework.Task)
...
func (q *queueContainer) GetWaitingTask(ctx context.Context, limit int32) (tasks []framework.Task, err error) {
...
func (q *queueContainer) ToRunningStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
	task.TaskStartTime = time.Now()
	task.TaskStatus = framework.TASK_STATUS_RUNNING
	if _, ok := q.runningTaskMap.LoadOrStore(task.TaskId, *task); !ok {
		atomic.AddInt32(&q.runningTaskCount, 1)
	}
	return task, nil
...
func (q *queueContainer) ToStopStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
	// 如果任务已经在执行中，删除执行中的任务
	...
	task.TaskStatus = framework.TASK_STATUS_STOPED
	return task, nil
}

// ToExportStatus 转移到删除状态
func (q *queueContainer) ToDeleteStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
	// 如果任务已经在执行中，删除执行中的任务
	if _, ok := q.runningTaskMap.LoadAndDelete(task.TaskId); ok {
		atomic.AddInt32(&q.runningTaskCount, -1)
	...
	task.TaskStatus = framework.TASK_STATUS_DELETE
	return task, nil
}

// ToFailedStatus 转移到失败状态
func (q *queueContainer) ToFailedStatus(ctx context.Context, task *framework.Task, reason error) (
	newTask *framework.Task, err error) {
	if _, ok := q.runningTaskMap.LoadAndDelete(task.TaskId); ok {
		atomic.AddInt32(&q.runningTaskCount, -1)
	}
	log.Printf("failed task %s, reason %v", task.TaskId, reason)
	task.TaskStatus = framework.TASK_STATUS_FAILED
	return
}

// ToExportStatus 转移到数据导出状态
func (q *queueContainer) ToExportStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
	// 删除执行中的任务，增加一个任务调度空位
	if _, ok := q.runningTaskMap.LoadAndDelete(task.TaskId); ok {
		atomic.AddInt32(&q.runningTaskCount, -1)
	}
	task.TaskStatus = framework.TASK_STATUS_EXPORTING
	return task, nil
}

// ToSuccessStatus 转移到执行成功状态
func (q *queueContainer) ToSuccessStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
	task.TaskStatus = framework.TASK_STATUS_SUCCESS
	return task, nil
}
