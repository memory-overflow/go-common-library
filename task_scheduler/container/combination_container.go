...
import (
	...
	persistcontainer "github.com/memory-overflow/go-common-library/task_scheduler/container/persist_container"
)

// combinationContainer 组合 MemeoryContainer 和 Persistcontainer 的容器，整个两种容器的优点，既能够通过内存实现快写快读，又能够通过DB实现可持久化
...
// AddTask 添加任务
func (c *combinationContainer) AddTask(ctx context.Context, task framework.Task) (err error) {
...
func (c *combinationContainer) GetRunningTask(ctx context.Context) (tasks []framework.Task, err error) {
...
func (c *combinationContainer) GetWaitingTask(ctx context.Context, limit int32) (tasks []framework.Task, err error) {
	return c.memeoryContainer.GetWaitingTask(ctx, limit)
}

// ToRunningStatus 转移到运行中的状态
func (c *combinationContainer) ToRunningStatus(ctx context.Context, task *framework.Task) (
...
func (c *combinationContainer) ToStopStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
...
func (c *combinationContainer) ToDeleteStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
...
func (c *combinationContainer) ToFailedStatus(ctx context.Context, task *framework.Task, reason error) (
	newTask *framework.Task, err error) {
...
func (c *combinationContainer) ToExportStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
...
func (c *combinationContainer) ToSuccessStatus(ctx context.Context, task *framework.Task) (
	newTask *framework.Task, err error) {
